class HCRAM {
    ...
    static func getRam() -> String {
        ...
        let ramType = run("grep 'Type' " + thesysmemfile + " | awk '{print $2}' | sed -n '1p'")
        print("RAM Type: " + ramType)
        let ramSpeed = run("grep 'Speed' " + thesysmemfile + " | grep 'MHz' | awk '{print $2\" \"$3}' | sed -n '1p'").trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    static func getMemDesc() -> String {
        return run("echo \"$(egrep \"ECC:|BANK |Size:|Type:|Speed:|Manufacturer:|Part Number:\" " + thesysmemfile + ")\"")
    }
