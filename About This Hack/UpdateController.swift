...
class UpdateController {
    ...
    static func updateATH() {
        ...
        _ = run("curl -L " + initGlobVar.lastAthreleaseURL + run("tr -d '[:space:]' <  " + initGlobVar.athtargetversionfile) + " " + initGlobVar.newAthziprelease + " -o " + initGlobVar.athDirectory + initGlobVar.newAthreleasezip)
        ...
        do {
            _ = try Zip.unzipFile(URL(string: " " + initGlobVar.alexanderskulaURL + initGlobVar.athfilesDirectory + initGlobVar.newAthreleasezip)!, destination: URL(string: " " + initGlobVar.alexanderskulaURL)!, overwrite: true, password: "")
        } catch {
            print(error)
        }
        ...
        _ = run("cp -rf " + NSHomeDirectory() + initGlobVar.thisAppliname + " " + initGlobVar.allAppliLocation)
